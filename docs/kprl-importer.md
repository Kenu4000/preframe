...
診断プレビューは複数の`.org`をまとめて生成し、生成済みSEEN間の`jump`を実行できます。本文リソースのない制御用シーンは同名`.utf`がなくても空のリソース集合として読み込みます。`.org`から実際に参照された本文またはtitleのリソースIDが存在しない場合は欠落IDを記録して続行します。厳密Importerは従来どおり`--resource`を要求し、欠落参照で失敗します。`.utf`だけが存在して対応する`.org`がない場合は診断プレビューも入力エラーにします。
